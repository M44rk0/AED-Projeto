...
class MapaTkinter(tk.Tk):
    def __init__(self):
        ...
        self.desenhar_grafo()
        # Configurar o callback do HistoryManager após todos os métodos estarem definidos
        self.after(0, lambda: setattr(self.history_manager, 'refazer_rota_callback', self.refazer_rota))
    ...
    def criar_painel_historico(self, parent):
        ...
        # Criar HistoryManager sem callback inicial
    ...
    def desenhar_grafo(self):
        # Sempre desenhar o canvas vazio com grid primeiro
        self.imgtk = self.graph_drawer.desenhar_canvas_vazio()
        
        if not self.graph_manager.existe_grafo():
            # Se não há grafo, mostrar botões centrais sobre o grid
            self.mostrar_botoes_centrais()
            return
            
        # Se há grafo, desenhar sobre o grid
    ...
    def gerar_vertices_aleatorios(self):
        ...
        if quantidade:
            ...
            # Ocultar botões centrais quando criar grafo
            self.ocultar_botoes_centrais()
            self.desenhar_grafo()
            self.atualizar_estado_botoes()
    ...
    def apagar_grafo(self):
        ...
        self.history_manager.historico_rotas.clear()
        self.history_manager.historico_capturas.clear()
        self.history_manager.atualizar_historico()
        self.desenhar_grafo()
        self.atualizar_estado_botoes()
    ...
    def mostrar_botoes_centrais(self):
        ...
        if self.graph_manager.existe_grafo() or self.modo_edicao:
            return
